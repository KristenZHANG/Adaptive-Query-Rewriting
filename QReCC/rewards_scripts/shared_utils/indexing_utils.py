...
class SparseIndexer:
    ...
    def get_psg_text(self, pid):
        gold_doc = self.searcher.doc(pid)
        return json.loads(gold_doc.raw())['contents']
    
    def get_psg_text_sentences(self, pid):
        def split_docs(doc):
            return nltk.sent_tokenize(doc)
        gold_doc = self.searcher.doc(pid)
        return split_docs(json.loads(gold_doc.raw())['contents'])

    def get_corpus(self, question, num_passages):
        def split_docs(doc):
            return nltk.sent_tokenize(doc)
        hits = self.searcher.search(question, k=num_passages)
        return [{"psg_id": hit.docid, "score": hit.score, "sentences": split_docs(json.loads(hit.raw)["contents"])} for hit in hits]

    def retrieve(self, question, num_passages, return_raw=False, return_text=True):
        hits = self.searcher.search(question, k=num_passages)
        
        if return_raw:
            return [json.loads(hit.raw) for hit in hits]
        
        if return_text: return [{"id": hit.docid, "score": hit.score, "text": json.loads(hit.raw)["contents"]} for hit in hits]
        else: return [{"id": hit.docid, "score": hit.score} for hit in hits]

    def compute_score(self, question, gold_doc_id):
        return self.index_reader.compute_query_document_score(gold_doc_id, question)

    def get_rank(self, question, num_passages, gold_doc_id):
        hits = self.searcher.search(question, k=num_passages)
        single_score = self.compute_score(question, gold_doc_id)
        single_score_last = self.compute_score(question, hits[99].docid)
        ...
        for i, hit in enumerate(hits):
            if hit.docid == gold_doc_id:
                passage_ranking = i + 1
                passage_score = hit.score
                # passage_content = json.loads(hit.raw)["contents"]
                break
        

        return {
            "question": question, 
            "ranking": passage_ranking, 
            "score": passage_score, 
            "single_score_distance": {
                '100': single_score_last,
                'gold': single_score,
                'difference': (single_score_last - single_score) / single_score if single_score >0 else None
            }
        }, {
            "id": gold_doc_id, 
            # "text": gold_text, 
            "status": passage_ranking is not None}, None
        # [hit.score for hit in hits]
    def get_rank_coarse(self, question, num_passages, gold_doc_id):
        hits = self.searcher.search(question, k=num_passages)

        single_score = None
        single_score_last = None
        difference = None
        ...
        for i, hit in enumerate(hits):
            if hit.docid == gold_doc_id:
                ...
                # passage_content = json.loads(hit.raw)["contents"]
                break
        if not passage_ranking:
            single_score = self.compute_score(question, gold_doc_id)
            single_score_last = self.compute_score(question, hits[99].docid) if len(hits) >= 100 else None
            difference = (single_score_last - single_score) / single_score if single_score >0 else None
        ...
        return {
            ...
            # "text": gold_text, 
            "status": passage_ranking is not None}, None
    def get_rank_nodiff(self, question, num_passages, gold_doc_id):
        hits = self.searcher.search(question, k=num_passages)
        single_score = None
        single_score_last = None
        difference = None
